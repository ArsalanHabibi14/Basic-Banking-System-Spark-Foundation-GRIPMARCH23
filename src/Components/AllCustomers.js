...
const AllCustomers = () => {
    ...
    >
      ...
      <td className="px-6 py-4">
        <button
          id={customer.id}
          onClick={handleModal}
          className=" bg-green-700  text-white hover:bg-blue-900 py-2 px-4 rounded-md"
        >
          Transfer
        </button>
      </td>
  ...
  return (
    <div
      // style={backImg}
      className="h-[53rem] relative bg-teal-700 w-full  bg-center bg-no-repeat bg-cover mt-12 md:mt-[4.5rem] before:content-[''] before:bg-gradient-to-br before:from-teal-700 before:absolute before:block before:h-full before:w-full"
    >
      {showModal && (
        <Modal
          customers={customers}
